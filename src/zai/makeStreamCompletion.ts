...
export async function makeStreamCompletion(response: Response, options: Partial<StreamOptions> | string = {}): Promise<void> {
  ...
  const config: StreamOptions = typeof options === "boolean" ? { sso: options, model: "", print: true } : { sso: false, print: true, model: "", ...(options as Partial<StreamOptions>) }
...
async function processLine(
  ...
  try {
    ...
    if (line.startsWith("data: ")) {
      ...
      const jsonData: StreamData = JSON.parse(jsonString)
 ...
 */
function convertToOpenaiTextStream(jsonData: StreamData, model: string, completionId: number): any | null {
