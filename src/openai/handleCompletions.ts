...
async function sendStreamResult(response: any, modelName: string, promptMode: boolean, c: Context) {
...
async function sendResult(response: any, modelName: string, promptMode: boolean, c: Context) {
  ...
  if (promptMode) {
    jsonResponse.choices = response.choices.map((choice: any, index: number) => ({
      index,
...
async function handleCompletions(chatRequest: ChatCompletionRequest, c: Context, io: any, chatHandler: any) {
  ...
  return streaming ? await sendStreamResult(response, modelName, promptMode, c) : await sendResult(response, modelName, promptMode, c)
}
