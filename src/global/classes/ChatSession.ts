...
export class ChatSession {
  ...
  private _useDatabase: boolean =
    process.env.CHAT_SESSION_USE_DATABASE === "true" ? true : false
  ...
  private _databaseHandler = {
    getRecordByChecksum: async (checksum: string) => {
      return await getChatByChecksum(checksum)
    },
    getRecordByChatId: async (chatId: string) => {
      return await getChatByChatId(chatId)
    },
    updateRecord: async (chatId: string, row: any) => {
      return await updateChat(chatId, row)
    },
    createRecord: async (row: any) => {
      return await saveChat(row)
    },
  }
  ...
  private async loadRegistry(): Promise<void> {
    ...
    if (this._useDatabase) {
      foundRegistry = await this._databaseHandler.getRecordByChecksum(
        hexChecksum
      )
    } else {
      ...
      if (this._chatRegistry) {
        if (!this._useDatabase) {
          ...
          const registryIndex = ChatSession._chatRegistries.findIndex(
            (reg) =>
              reg.chatId === this._chatId || reg.chatId === this._oldChatId
          )
          ...
          const { lastAssistantMessageId, lastUserMessageId, checksum } =
            this._chatRegistry
