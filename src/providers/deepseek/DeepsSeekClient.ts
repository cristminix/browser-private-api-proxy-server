...
class DeepsSeekClient {
          ...
          try {
            ...
            if (line.startsWith("data: ")) {
              ...
              if (jsonData.v?.response?.fragments?.[0]?.type) {
                ...
                jsonData.p === "response/fragments" &&
                Array.isArray(jsonData.v) &&
                jsonData.v[0]?.type
              ) {
  ...
  convertToOpenaiTextStream(
    ...
    if (ip === "response/status" && oo === "SET" && inputData === "FINISHED")
      return null;
    ...
    if (typeof inputData === "object" && !Array.isArray(inputData)) {
      ...
      if (response && response.fragments) {
        for (const fragment of response.fragments) {
          if (
            (fragment.type === "RESPONSE" || fragment.type === "THINK") &&
            fragment.content
          ) {
